...
function updatePriceDisplay(currentPrice) {
	...
	document.getElementById('time-change').innerHTML = getLabel(timeframe) +":";
...
async function updateChartData() {
    ...
    const timeframe =document.getElementById('primaryTimeframe').value || '4h';
    const limit = getLimit(timeframe);

    try {
        updateConnectionStatus('warning', 'Actualizando...');
        // Obtener los datos de las velas (candles originales)
        const rawCandles = await fetchRawCandleData(symbol, timeframe, limit);	
        const chartDataClose = toChartDataClose(rawCandles);		
		const chartDataEma10 = toChartDataEma10(rawCandles);
		const chartDataEma25 = toChartDataEma25(rawCandles);
		const chartDataEma50 = toChartDataEma50(rawCandles);
		const chartDataEma100 = toChartDataEma100(rawCandles);
		const chartDataEma200 = toChartDataEma200(rawCandles);		
        ...
        updateDataTable(rawCandles);  
        updatePriceDisplay(currentPrice);        
        updateConnectionStatus('success', 'Conectado');
        const now = new Date();
        document.getElementById('timestamp').textContent = now.toLocaleTimeString();
		priceChart.options.scales.x.display = !isMobileDevice(); // si es movil oculta leyenda eje tiempo grafica
		
		const lastIndex = candlesData.length - 1;
		
		firstPrice = candlesData[0].close;
		firstTime = candlesData[0].closeTime;
		lastPrice = candlesData[lastIndex].close;
		lastTime = candlesData[lastIndex].closeTime;
		
		
		const priceElement2 = document.getElementById('current-price-2');
		...
		const diffForm = Math.abs(diff).toFixed(2);
		
		const FForm = Math.abs(firstPrice).toFixed(2);
		const LForm = Math.abs(lastPrice).toFixed(2);
		...
		priceElement2.textContent = diffForm;

		if (change2 > 0) {
			priceElement2.className = 'price-positive';
			changeElement2.textContent = `+${changeFormatted2}%`;
			changeElement2.className = 'text-success';
		} else if (change2 < 0) {
			priceElement2.className = 'price-negative';
			changeElement2.textContent = `-${changeFormatted2}%`;
			changeElement2.className = 'text-danger';
		} else {
			priceElement2.className = '';
